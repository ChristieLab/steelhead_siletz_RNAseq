...
#####==========Hypo prediction plot; not real data ========================================#

plot(c(1,2,3,4), c(HH, HH, WW, WW), bty = "l", xaxt='n', ylab = "Standardized log gene counts", xlab="", xlim = c(0.75, 4.25), ylim = c(6.1, 6.5))
perfect <- (HH+WW)/2
...
plot(c(1,2,3,4), c(HH, HW, WH, WW), bty = "l", xaxt='n', ylab = "Standardized log gene counts", xlab="", xlim = c(0.75, 4.25), ylim = c(6.1, 6.5))
...
plot(rep(1, nrow(diffs4)), diffs4[, 1], bty="l", ylab="Standardized log gene counts", pch=19, col = alpha("blue", 0.1), cex=1.2,  xaxt='n', xlab="", xlim = c(0.75, 4.25), ylim = c(6.1, 6.5))
...
for(n in 1:10000){ # number of replicate data sets
  ...
  OUT1 <- NULL
  for(n in 1:290){ # number of genes
